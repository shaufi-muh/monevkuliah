        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ route('jurusan.pertanyaan.update', $pertanyaan->id) }}" method="POST">
                        @csrf
                        ...
                        <div class="mt-4">
                            ...
                            <select name="kuisioner_id" id="kuisioner_id" class="block mt-1 w-full border-gray-300 ... rounded-md shadow-sm" required>
                                ...
                                @foreach ($kuisioners as $kuisioner)
                                    <option value="{{ $kuisioner->id }}" {{ old('kuisioner_id', $pertanyaan->kuisioner_id) == $kuisioner->id ? 'selected' : '' }}>
                        ...
                        <div class="mt-4">
                            ...
                            <textarea name="isi_pertanyaan" id="isi_pertanyaan" rows="3" class="block mt-1 w-full border-gray-300 ... rounded-md shadow-sm" required>{{ old('isi_pertanyaan', $pertanyaan->isi_pertanyaan) }}</textarea>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                            <div>
                                ...
                                <select name="tipe_jawaban" id="tipe_jawaban" class="block mt-1 w-full border-gray-300 ... rounded-md shadow-sm" required>
                                    <option value="numerik" {{ old('tipe_jawaban', $pertanyaan->tipe_jawaban) == 'numerik' ? 'selected' : '' }}>Numerik (Angka)</option>
                                    <option value="boolean" {{ old('tipe_jawaban', $pertanyaan->tipe_jawaban) == 'boolean' ? 'selected' : '' }}>Pilihan (Ya/Tidak, Sesuai/Tidak)</option>
                                </select>
                            </div>
                            <div>
                                <x-input-label for="urutan" :value="__('Nomor Urut Tampil')" />
                                <x-text-input id="urutan" class="block mt-1 w-full" type="number" name="urutan" :value="old('urutan', $pertanyaan->urutan)" required />
                            </div>
                        </div>

                        <div class="flex items-center justify-end mt-6">
                            <a href="{{ route('jurusan.pertanyaan.index', ['kuisioner_id' => $pertanyaan->kuisioner_id]) }}" class="... mr-4">Batal</a>
