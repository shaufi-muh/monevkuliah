<x-app-layout>
    ...
    <div class="py-12" x-data="{
        ...
        initForm() {
            ...
            const kIdInput = form ? form.querySelector('#kuisioner_id') : null;
            const kId = kIdInput ? kIdInput.value : '';
            if (this.open && kId) {
                document.getElementById('modal-title').innerText = 'Edit Set Kuisioner';
                form.action = '{{ url('jurusan/kuisioner') }}/' + kId;
                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <div class="mb-4 text-right">
                        <button type="button" @click="
                            ...
                            if(form){ form.action = '{{ route('jurusan.kuisioner.store') }}'; form.reset(); }
                        " class="inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-500 focus:outline-none focus:border-green-700 focus:ring ring-green-300 transition ease-in-out duration-150" title="Tambah Kuisioner Baru">
                    ...
                    <div x-show="open" style="display: none;" x-on:keydown.escape.window="open = false" class="fixed z-10 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                            ...
                            <div x-show="open" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
                                <form id="kuisionerForm" action="{{ route('jurusan.kuisioner.store') }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="kuisioner_id" id="kuisioner_id" value="{{ old('kuisioner_id') }}">
                                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                        <h3 class="font-semibold text-lg mb-4" id="modal-title">Buat Set Kuisioner Baru</h3>
                                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            <div>
                                                <label for="tahun_akademik" class="block font-medium text-sm text-gray-700">Tahun Akademik</label>
                                                <input id="tahun_akademik" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm" type="text" name="tahun_akademik" value="{{ old('tahun_akademik') }}" maxlength="9" pattern="\d{4}\/\d{4}" placeholder="Contoh: 2024/2025" required />
                                            ...
                                            <div>
                                                <label for="sesi" class="block font-medium text-sm text-gray-700">Sesi</label>
                                                <select name="sesi" id="sesi" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm">
                                                    <option value="Tengah" {{ old('sesi') == 'Tengah' ? 'selected' : '' }}>Tengah Semester</option>
                                                    <option value="Akhir" {{ old('sesi') == 'Akhir' ? 'selected' : '' }}>Akhir Semester</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="mt-4">
                                ...
                                <tr>
                                    ...
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <button type="button" @click="
                                            open = true;
                                            let form = document.getElementById('kuisionerForm');
                                            let kData = {{ Illuminate\Support\Js::from($kuis) }};
                                            form.action = '{{ url('jurusan/kuisioner') }}/' + kData.id;
                                            ...
                                            document.getElementById('modal-title').innerText = 'Edit Set Kuisioner';
                                            form.querySelector('#tahun_akademik').value = kData.tahun_akademik || '';
                                            form.querySelector('#semester').value = kData.semester || '';
                                            form.querySelector('#sesi').value = kData.sesi || '';
                                            form.querySelector('#deskripsi').value = kData.deskripsi || '';
                                            form.querySelector('#status').value = kData.status || '';
                                            form.querySelector('#kuisioner_id').value = kData.id;
